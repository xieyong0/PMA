...
def Margin_loss(logits, y, y_target):
    device = y.device
    target_class = logits.shape[-1]

    loss = ((torch.eye(target_class)[y.cpu()]).to(device)* logits).sum(dim=-1) -  ((torch.eye(target_class)[y_target.cpu()]).to(device)* logits).sum(dim=-1)
    return loss.squeeze()

class APGD_AT():
    def __init__(self, model, n_iter=100, norm='Linf', n_restarts=1, eps=None,
                 seed=0, eot_iter=1, rho=.75, verbose=False, step_size=2./255, loss = "CE"):
        self.model = model
        self.n_iter = n_iter
        self.eps = eps
        self.norm = norm
    ...
    def attack_single_run(self, x_in, y_in):
        ...
        output = self.model(x)
        target_class = output.shape[-1]
        one_hot = torch.eye(target_class)[y.cpu()]
        y_target = ((1-one_hot).to(device)*output).argmax(dim=-1)
        ...
        for _ in range(self.eot_iter):
            with torch.enable_grad():
                ...
                if self.loss == 'CE':
                    loss_indiv = nn.CrossEntropyLoss(reduce=False, reduction='none')(logits, y_target)
                elif self.loss == 'Margin':
                    loss_indiv = -Margin_loss(logits,y,y_target)
                elif self.loss == 'Dlr':
        ...
        step_size = self.step_size * torch.ones([x.shape[0], 1, 1, 1]).to(device).detach() * torch.Tensor([1.0]).to(device).detach().reshape([1, 1, 1, 1])
            ...
            with torch.no_grad():
              ...
              if counter3 == k:
                  ...
                  if np.sum(fl_oscillation) > 0:
                      ...
                      one_hot = torch.eye(target_class)[y.cpu()]+torch.eye(target_class)[y_target.cpu()]
                      y_target = ((1-one_hot).to(device)*output).argmax(dim=-1)
