    ...
    def perturb(self, x_in, y_in):
        ...
        for r in range(self.num_restarts):
            ...
            for i in range(self.num_steps):
                ...
                #opt.zero_grad()
                logits = model(X_pgd)
                y_t_one_hot = (1 - torch.eye(self.num_classes)[y_in.cpu()]).to(device)
                y_t = (logits * y_t_one_hot).argmax(dim=-1)

                if i < self.num_steps/3:
                    loss = F.cross_entropy(logits,y_in)
                elif i<self.num_steps/3*2:
                    loss = -F.cross_entropy(logits,y_t)
                else:
                    loss = torch.sum(logits[y_t]-logits[y_in])
