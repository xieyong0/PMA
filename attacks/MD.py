            ...
            for r in range(2):
                ...
                for i in range(self.num_steps):
                    ...
                    with torch.enable_grad():
                        ...
                        p_max, max_idx = torch.max(logits * (1 - y_gt) - y_gt * 1.e8, dim=1)
                        p_y, _ = torch.max(logits * y_gt - (1 - y_gt) * 1.e8, dim=1)
                        ...
                        if self.use_odi and i < 2:
                            ...
                            loss_per_sample = p_max if r else -p_y
                            loss = torch.mean(loss_per_sample)
                        elif self.use_dlr:
                            loss = self.dlr_loss(logits, y).mean()
                        else:
                            loss_per_sample = p_max - p_y
                            loss = torch.mean(loss_per_sample)
                        X_adv, nc = adv_check_and_update(X_pgd.detach(), logits, y, nc, X_adv)
                        loss.backward()
